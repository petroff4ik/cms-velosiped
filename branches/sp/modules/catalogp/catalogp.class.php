...
class catalogp extends add_module {
    ...
    function create_table() {
        ...
        xlsWriteLabel(0, 2, "descr");
        xlsWriteLabel(0, 3, "price");
        xlsWriteLabel(0, 4, "lang");
        $xlsRow = 1;
        foreach ($menu as $key => $value) {
            xlsWriteNumber($xlsRow, 0, iconv("utf-8", "windows-1251", $value['idcp']));
            xlsWriteLabel($xlsRow, 1, iconv("utf-8", "windows-1251", $value['name']));
            xlsWriteLabel($xlsRow, 2, iconv("utf-8", "windows-1251", $value['descr']));
            xlsWriteLabel($xlsRow, 3, iconv("utf-8", "windows-1251", $value['price']));
            xlsWriteLabel($xlsRow, 4, iconv("utf-8", "windows-1251", $value['lang']));
            $xlsRow++;
        }
        xlsEOF();
    ...
    function _updateCsv(){
        set_time_limit(380);
		$f = fopen(SITE_DIR . '/templates/upload/menuupdate.csv', 'r');
		$data = fgetcsv($f, 1000, '|');
		while (!feof($f)) {
			$data = fgetcsv($f, 1000, '|');
			list($cpid, $name, $descr, $price, $lang) = $data;
                        if(is_numeric($cpid)){
                            if(empty($name)){
                              echo "Delete $cpid, $name, $descr, $price, $lang <br/>";
                              $this->model->delSpec($cpid, $name, $descr, $price, $lang);
                            }else{
                              echo "Update $cpid, $name, $descr, $price, $lang <br/>";
                              $this->model->updateSpec($cpid, $name, $descr, $price, $lang);
                            }
                        }else{
                            echo "Problem $cpid, $name, $descr, $price, $lang <br/>";
                        }
		}
