...
class catalogp_model extends add_model_module {
	...
	function get_all_branch_site($id_nstree) {
		return $this->db->result_array($this->db->query("SELECT * FROM catalogp, id_lang_text,ns_doc WHERE catalogp.type='br' AND name_alias=id_lang_text.id AND  id_lang_text.lang='" . $GLOBALS['lang'] . "' AND ns_doc.show_me=1 AND ns_doc.father_id='" . $id_nstree . "' AND ns_doc.id=catalogp.id_doc ORDER BY pos;"));
	}
