...
class file_browser {
    ...
    function bytesize($size) {
        if (empty($size))
            return number_format($size, 2) . " KB"; $size = $size / 1024;
        if ($size < 1024)
    ...
    function error($code) {
        switch ($code) {
            case 'ERR01': $error = 'Invalid interface path. Please make a proper call for the File Browser Class.';
                break;
            case 'ERR02': $error = 'Valid interface path but the template you have selected does not exist.';
                break;
        }
        if (isset($error)) {
    ...
    function row($templates, $type, $path, $title, $adtype) {
        ...
        switch ($title) {
            case '.': $filename['title'] = '[.]';
                ...
                $filename['link'] = ADMIN_DIR_FULL."edittemplate" . "?dir=" . base64_encode(realpath($path));
            ...
            case '..': $filename['title'] = '[..]';
                ...
                $filename['size'] = "--- ---";
                $filename['icon'] = "folder";
                $filename['link'] = ADMIN_DIR_FULL."edittemplate" . "?dir=" . base64_encode(realpath($path . '/' . $title));
                $filename['update'] = date("F dS, Y - H:i", filemtime($path . "/" . $title));
                break;
            default: $filename['title'] = $title;
                ...
                if (empty($type)) {
                    ...
                    $filename['link'] = ADMIN_DIR_FULL."readtemplate?filename=". realpath($path . '/' . $filename['title']);
                    $filename['update'] = date("F dS, Y - H:i", filemtime($path . "/" . $title));
                } else {
                    $filename['type'] = (empty($type) ? $this->filetypes($properties['extension'], $adtype, $hidden) : "Folder");
                    $filename['size'] = $this->bytesize(filesize($path . "/" . $title));
                    $filename['icon'] = "folder";
                    $filename['link'] = ADMIN_DIR_FULL."edittemplate" . "?dir=" . base64_encode(realpath($path . '/' . $filename['title']));
                    $filename['update'] = date("F dS, Y - H:i", filemtime($path . "/" . $title));
                }
                break;
        }
    ...
    function __construct($interface, $template, $chrooted=false, $adtype=true) {
        ...
        if (!is_dir($interface))
            $this->error('ERR01');
        if (!include($interface . '/template-' . $template . '.php'))
            $this->error('ERR02');
        ...
        if (empty($select) && !empty($_GET['select']))
            $select = $_GET['select']; else
            $select=null;
        if (empty($directory) && !empty($_GET['dir']))
            $directory = base64_decode($_GET['dir']);
        if (empty($chrooted) OR !is_dir(realpath($chrooted)))
            $chrooted = realpath('./'); else
            $chrooted=realpath($chrooted);
        if (empty($directory))
            $directory = $chrooted;
        
        $directory = realpath($directory);
        if (substr($directory, 0, strlen($chrooted)) != $chrooted)
            $directory = $chrooted;
        if (!empty($select))
            $filter = true; else
            $filter=false;
        ...
        foreach ($scan as $scanned) {
            if (is_dir($directory . "/" . trim($scanned)))
                $inarray = "folders"; else
                $inarray="files";
            switch ($select) {
                case null: $scans[$inarray][] = trim($scanned);
                    break;
                case "numbers": if (!in_array(strtolower(substr(trim($scanned), 0, 1)), $letters))
                        $scans[$inarray][] = trim($scanned); break;
                default: if (strtolower(substr(trim($scanned), 0, 1)) == strtolower($select))
                        $scans[$inarray][] = trim($scanned); break;
            }
        }
        ...
        if (count($scans['folders']) + count($scans['files']) == 0)
            $templates->layout_nofiles($filter);
        
        if ($this->displaydot == false && empty($select))
            array_shift($scans['folders']);
        foreach ($scans['folders'] as $title)
            $this->row($templates, true, $directory, $title, $adtype);
        foreach ($scans['files'] as $title)
            $this->row($templates, false, $directory, $title, $adtype);
