...
define("E_VAL_NEELMNT_cap", "Empty fields");
define("E_VAL_NEELMNT_cap2", "Wrong symbol");
...
class FormValidator {
    ...
    protected $validator_array;
    protected $error_hash;
    protected $custom_validators;
    protected $desct = true;

    function __construct($arg=true) {
        $this->desct = $arg;
        ...
        if (!empty($_SESSION['validate'])) {
            foreach ($_SESSION['validate'] as $key => $value)
                if (!empty($key))
                    $this->$key = $value;
        };
    }
    ...
    function ValidateCommand($command, $command_value, $input_value, &$default_error_message, $variable_name, $formvariables) {
        ...
        // echo "<br />".$input_value."->".$variable_name;
