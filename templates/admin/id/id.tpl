		...
		<form method="post" action="<?= $admin_dir_full; ?>createinvfortree/<?= $idc; ?> ">
			<select name="iddoc"  >
				<? foreach ($sch2 as $keyi => $valuei): ?>
					<option value="<?= $valuei['id']; ?>"><?= $valuei['class']; ?>(<?= $valuei['namet']; ?>) <?= $valuei['types']; ?><? echo @mb_substr(htmlspecialchars($valuei['private_info']['text']), 0, 30, 'UTF-8');?></option>
				<? endforeach; ?>
			</select>
			<br/>
							...
							<form  action="<?= $admin_dir_full; ?>saveSideDoc/<?= $value['id']; ?>/<?= $idc; ?>" method="post">
								...
								<select size="1" name="sidedoc">
									<? if ($value['sch']): ?>
										...
										<? foreach ($all_side As $key2 => $value2): ?>
											<? if ($value['side_for_doc'] != $value2['side']): ?>
												<option value="<?= $value2['side']; ?>"><?= $value2['side']; ?>(<?=$value2['name'];?>)</option>
											<? endif; ?>
										<? endforeach; ?>
									<? else: ?>
										<option value="<?= $value['side_for_doc']; ?>"><?= $value['side_for_doc']; ?></option>
