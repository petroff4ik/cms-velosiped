...
class add_contrl extends api {
		...
		try {
			if (file_exists($path)) {
				if ($xml = !@simplexml_load_file($path))
					throw new Exception($this->lang['l_error_load_module']);

				chdir(SITE_DIR . "modules/");
				if (mkdir(((string) $xml->module_info->name_class), 0777))
					throw new Exception($this->lang['l_error_load_module_mkdir']);;
	...
	function instal_template($path) {
		...
		try {
			if (file_exists($path)) {
				if ($xml = !@simplexml_load_file($path))
					throw new Exception($this->lang['l_error_load_template']);
	...
	function startLaunch($id, $lang, $order="", $vector="", $pag="", $search="", $mod_order="") {
		...
		foreach ($data['sch2'] as $key2 => $value2)
			$data['sch2'][$key2]['private_info'] = $this->_call_out_func($value2['module'], "_returnLink", $value2['id'], $lang);
		foreach ($data['sch3'] as $key3 => $value3)
			$data['sch3'][$key3]['private_info'] = $this->_call_out_func($value3['module'], "_returnLink", $value3['id'], $lang);
		$data['sch'] = array();
		if (!empty($sch))
